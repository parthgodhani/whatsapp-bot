...
var whatsApp = async ({ target, session }) => {
    const config = {
        puppeteer: {
            executablePath: 'C:/Program Files/Google/Chrome/Application/chrome.exe', args: [
                '--incognito',
            ],
        },
    }
    ...
    mqttClient.on("connect", () => {
        ...
        client.on("authenticated", (result) => {
            ...
            console.log(result);
            console.log("result")
            mqttClient.publish('authenticated', JSON.stringify({
                type: "READY",
                data: result
            }));
        })
        client.on('ready', async (data) => {
            ...
            // send video
            const media = MessageMedia.fromFilePath('C:/Users/01/Downloads/file_example_MP4_480_1_5MG.mp4');
            // const media = MessageMedia.fromFilePath('C:/Users/01/Downloads/Par Infotech Public Holiday List (1).docx');

            target.map(t => { return sendMedia(client, t, media) });
...
const sendMsg = async (client, t) => { client.sendMessage(t, "Test msg from whatsapp automation") }
const sendMedia = async (client, t, media) => { var chat = await client.getChatById(t); chat.sendMessage(media, { caption: "Test msg with caption from whatsapp automation" }) }
whatsApp(workerData);